...
alt+ctrl+F2   	/*goes to gui*/					alt+ctrl+F4   /*goes to tty*/
/*send message to a user*/
mesg y 		- mesg n
1.	who -T	/*show all users*/		2.write babak pts/0		/*after this command write message*/
wall 		/*send meesage to all users*/
notify-send "alert title" "body of notification"
sleep 10; notify-send "alert title" "body of notification" 	/*for remembering*/

DISPLAY=:0 sudo su -c "notify-send \"alert title\" \"body of notification\"  					"
/*یه دستور نوتیفای دادیم برای نمایشگر 0 که از دستور who گرفتیمش. -c هم یعنی کامند*/

/*backup giri*/
tar -cfz /tmp/backupOfFile.tar.gz /home/babak/data/ 		/*-c is create 	-f is file  -z gzip   -j bzip2 */
tar -tf backupOfFile 		/*shows list of files in backupOfFile.tar*/
tar -uf /tmp/backupOfFile.tar.gz /home/babak/data/ 			/*-u update tar file*/
rsync -avh	/home/babak/data /tmp 							/*-a archive    increment backup/copy*/
rsync -avh /home/babak/ backup@192.168.1.50:/var/backup 			/*backup to a server*/

/*Compile*/
1-  ./file.sh 	2- ./configure 		3- make 		4- sudo make install 	5-/*copy missing lib into /usr/lib64*/ 

/****************************Monitoring******************************/
htop 		systemctl enable sysstat	/*to enable sar*/	
sar /*best command*/		sar 1 4 	/*هر یک ثانیه ۴ بار نتایجو بگیر*/
/*memory*/
free 	/*shows memory status*/ 		
vmstat 	/*shows status of virtual memory or buffer*/ 	pstree
/*cpu*/
mpstat 	/*status of multi cpu*/ 	iostat 	/*status of cpu*/
w 	/*show users cpu using stats and what is doing now*/ 		ps 
/*network*/
iftop 	/*چه چیزایی دارن نتورک مصرف میکنن*/ 	sudo iftop -i wlp4s0 	/*i inteface*/
sudo iptraf -ng 	/*مثل بالایی ولی بهتر و مدرن تر*/
tcpdump /*تمام کاراهایی که در شبکه درحال انجام است را در یک فایل مینویسد برای رفع اشکال یا مهندسی معکوس*/
ss /*shows all sockets*/		ntop 		netstat 		ip
/*disk*/
iostat 		- 	sudo iotop 		-	fuser	- 	lsof 		- 	df 	-h	-	du
/*****Monitoring softwares*****/
cacti 		mrtg 		nagios 	 	munin /*best*/ 		collectd

/*****File systems*****/
modprob iwlmvm 	/*enable it*/ 		modprob -r iwlmvm 	  /*remove it*/
mkfs -t ext4 /dev/sdb1 		/*-t is type*/
mount /dev/sdb1 /mnt 			mount file.iso /mnt
swapon -a 	/*on all swaps from /etc/fstab*/

/*tools for encrypting disks*/
dm-crypt 		cryptsetup		
ecryptfs-utils 		 /*(best)*/
	1. mount /dev/sdb1 /mnt 		2. mount ecryptfs /mnt /mnt
e2lable /dev/sdb1			/*for labeling disk*/
debugfs /dev/sdb1
lsblk 	/*list of devices block*/
blkid /dev/sdb1  /*shows uuid of sdb1*/ 		resize2fs
uuidgen 	/*generates a uuid*/		tune2fs /dev/sdb1 -U <uuid generated by uuidgen> 		/*set new uuid*/
fsck /dev/sda1		/*checks file system*/

/******network******/
ifup device 	- 	ifdown device 	 							/*enable and disable network device*/
sudo ifconfig device up 192.168.1.2 netmask 255.255.255.0		/*set ip for device network*/
ip addr add 192.168.1.2/24 dev device 							/*more modern than ifconfig up*/
ip link set device down   										/*disable network device*/
sudo ifconfig device hw ethernet AA:BB:CC:DD:EE:FF 				/*change mac address*/
ip addr show 		- 	ip route 		- 	ip route del default 	- 	ip route add default via 192.168.1.1
iwlist device scan 		/*scan wifi networks*/		iwconfig device essid "clercks" key s:password 		/*connect to a wifi network*/
iw 						/*newer command*/
netstat -nr 		route -n 	 				/*routing table*/
netstat -tl 		/*کی داره پورتهای tcp رو گوش میده*/ 		netstat -tulpen
sudo route del default gw 192.168.1.1 			/*delete default gateway*/		sudo route add default gw 192.168.1.1
dhclient -v device 		/*show dhcp server of network device*/
arp -n 					/*shows ip of network hardware by mac address*/ 
/***Troubleshouting network***/
	1. ping gateway             ->ex: ping 192.168.1.1 	
	2. ping destination			->ex: ping google.com
	3. traceroute destination	->ex: traceroute google.com 		or 		mtr -n google.com 		/*-n for showing ip*/
nc -l 1234 	/*listen to port 1234*/		nc localhost 1234 	/*send packet or message from me to port 1234*/ tcpdump -i any port 1234 -A -w file.pcap 	/*شنود*/
nmap -v google.com 		/*for hacking - checks open ports of destination*/ /*میشود آی پی سرور خودم را چک کنم تا پورت اضافی باز نباشد*/
nmap -v 192.168.1.1/24 -p 23 	/*گوش بده به شبکه من تمام رنج آی پی موجود و روی پورت تلنت	*/
nslookup google.com 4.2.2.4     /*از ۴.۲.۲.۴ بپرس آی پی گوگل چیه*/ 		dig 	-	host 	
